...
class MusicController:
    ...
    def __init__(self):
        ...
        self._model.addTransition(8, [BTN3_RELEASE], 5)
        ...
        self._model.addTransition(6, [BTN1_PRESS], 8)
        self._model.addTransition(8, [BTN1_RELEASE], 6)
    ...
    def stateEntered(self, state, event):
        ...
        if state == 0:
            self._display.showText(f'Playing: {type(self._instrument).__name__}   ', row = 0)
            self._display.showText(f'Note Val: None  ', row = 1)
        
        elif state == 1:
            # entry actions for state 1
            self._buzzer.play(self._instrument.getNote(0))
            self._display.showText(f'Note Val: {self._instrument.getNote(0)}  ', row = 1)
        elif state == 2:
            # entry actions for state 2
            self._buzzer.play(self._instrument.getNote(1))
            self._display.showText(f'Note Val: {self._instrument.getNote(1)}  ', row = 1)
        elif state == 3:
            # entry actions for state 3
            self._buzzer.play(self._instrument.getNote(2))
            self._display.showText(f'Note Val: {self._instrument.getNote(2)}  ', row = 1)
        elif state == 4:
            # entry actions for state 4
            self._buzzer.play(self._instrument.getNote(3))
            self._display.showText(f'Note Val: {self._instrument.getNote(3)}  ', row = 1)
        elif state == 5:
            # entry actions for state 5
            self._buzzer.play(self._instrument.getNote(4))
            self._display.showText(f'Note Val: {self._instrument.getNote(4)}  ', row = 1)
        elif state == 6:
            # entry actions for state 6
            self._buzzer.play(self._instrument.getNote(5))
            self._display.showText(f'Note Val: {self._instrument.getNote(5)}  ', row = 1)
        elif state == 7:
            # entry actions for state 7
            self._buzzer.play(self._instrument.getNote(6))
            self._display.showText(f'Note Val: {self._instrument.getNote(6)}  ', row = 1)
        elif state == 8:
            # entry actions for state 8
            self._buzzer.play(self._instrument.getNote(7))
            self._display.showText(f'Note Val: {self._instrument.getNote(7)}  ', row = 1)
        elif state == 9:
            # entry actions for state 9
